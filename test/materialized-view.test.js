...
test("materialized view persists staged entries without exposing committed data", { concurrency: false }, async () => {
  const stagedOperation = {
    ...
    index: 8,
    prevIndex: 7,
    prevHash: "prev-staged-7",
    entryHash: "staged-hash-8",
    keyspace: "default",
    key: "hash:staged",
    actor: "node-a",
    seq: 8,
...
test("materialized view commits staged entries in one transition", { concurrency: false }, async () => {
  const operation = {
    ...
    index: 2,
    prevIndex: 1,
    prevHash: "prev-committed-1",
    entryHash: "committed-hash-2",
    keyspace: "default",
    key: "hash:committed",
...
test("materialized view skips staged entries in one transition", { concurrency: false }, async () => {
  const operation = {
    ...
    index: 5,
    prevIndex: 4,
    prevHash: "prev-skipped-4",
    entryHash: "skipped-hash-5",
    keyspace: "default",
    key: "hash:skipped",
    value: { ciphertext: "encrypted" },
    actor: "node-a",
...
test("materialized view does not restage entries behind committed progress", { concurrency: false }, async () => {
  const operation = {
    ...
    index: 3,
    prevIndex: 2,
    prevHash: "prev-already-2",
    entryHash: "already-hash-3",
    keyspace: "default",
  ...
  try {
    ...
    await first.view.stageEntry("feed-a", {
      ...
      operation: {
        ...
        index: 4,
        prevIndex: 3,
        prevHash: "prev-restart-3",
        entryHash: "staged-restart-hash-4",
        keyspace: "default",
